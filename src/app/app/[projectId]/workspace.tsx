                ...
                <div className="relative aspect-[9/16] w-full overflow-hidden bg-zinc-100">
                  ...
                  <button
                    ...
                    onClick={() => {
                      if (
                        !window.confirm(
                          `Hapus screenshot layar ${idx + 1} dari proyek ini?`,
                        )
                      ) {
                        return;
                      }
                      deleteShotMut.mutate(s.id);
                    }}
                    className="absolute right-2 top-2 rounded-md bg-white/95 p-1.5 text-zinc-500 shadow-sm opacity-0 ring-1 ring-zinc-200/80 transition-opacity hover:bg-red-50 hover:text-red-600 group-hover:opacity-100 disabled:opacity-50"
        ...
        {project!.screenshots.length > 0 && (
          <div className="rounded-xl border border-zinc-100 bg-zinc-50 p-4">
            ...
            <div className="flex gap-2 overflow-x-auto">
              {project!.screenshots.map((s, idx) => (
                <div key={s.id} className="group/shot relative shrink-0 w-12 h-20 rounded-lg overflow-hidden border border-zinc-200 bg-white">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
